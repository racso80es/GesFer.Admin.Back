...
3.  **GIT:** 🚫 NO commits a `main`. 🚫 NO ramas sin documentación. 🚫 **NO ejecutar comandos git directamente:** toda operación git ha de pasar por una skill, herramienta, acción o proceso (norma SddIA/norms/git-via-skills-or-process.md).
4.  **COMANDOS:** 🚫 **NO ejecutar comandos de sistema directamente** (git, dotnet, npm, pwsh, cargo, etc.). Toda ejecución ha de pasar por skill, herramienta, acción o proceso (norma SddIA/norms/commands-via-skills-or-tools.md). Implementación estándar: **Rust** (paths.skillsRustPath, paths.toolsRustPath; Cúmulo); **binarios en la raíz de la cápsula**; la **IA invoca el `.exe`** con **JSON stdin/stdout** según **SddIA/norms/capsule-json-io.md** (los `.bat` son opcionales para uso humano).
...
| **validate-pull-requests** | Revisión integral S+ Grade de un PR (rama origen): consenso architect, qa-judge, security-engineer; semillas Kaizen en paths.tasksPath. | `paths.featurePath/validate-pull-requests-<pr-slug>`; Kaizen en `paths.tasksPath` | [`SddIA/process/validate-pull-requests/`](./SddIA/process/validate-pull-requests/) |
...
| **[VALIDATE-PR]** | Revisión de PR, validar pull request, S+ Grade review, consenso multi-agente sobre rama del PR. | Cargar [`SddIA/process/validate-pull-requests/`](./SddIA/process/validate-pull-requests/) (spec.md con frontmatter). Orquestar architect, qa-judge y security-engineer. |
