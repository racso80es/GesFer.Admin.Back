...
public class AuditLogServiceTests
{
    ...
    public async Task LogActionAsync_ShouldCreateAuditLog_WhenCalledValidly()
    {
        ...
        var log = await context.AuditLogs.FirstOrDefaultAsync(System.Threading.CancellationToken.None);
    ...
    public async Task LogActionAsync_ShouldHandleNullAdditionalData_AndSetIsActive()
    {
        ...
        using var scope = _serviceProvider.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<AdminDbContext>();
        var log = await context.AuditLogs.FirstAsync(System.Threading.CancellationToken.None);
        log.Should().NotBeNull();
