<#
.SYNOPSIS
    Ejecuta la acción finalize: comprueba precondiciones e invoca la skill FinalizarProceso (Push-And-CreatePR).
.DESCRIPTION
    Orquestador de la acción finalize (SddIA/actions/finalize). Comprueba rama, objectives.md y validacion.json;
    opcionalmente ejecuta verify-pr-protocol; luego invoca la skill FinalizarProceso (Push-And-CreatePR.ps1)
...
try {
    ...
    if (-not (Test-Path $skillCapsule)) {
        Write-Error "No se encontró la skill FinalizarProceso: $skillCapsule (paths.skillCapsules['finalizar-proceso'])"
        exit 1
    }

    Write-Host "[Finalize] Invocando skill FinalizarProceso (Push-And-CreatePR) con -Persist $Persist" -ForegroundColor Cyan
