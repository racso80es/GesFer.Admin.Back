...
| :--- | :--- | :--- |
| **0** | Preparar entorno | Crear rama feat/<nombre_feature> (o `fix/` si aplica) desde `master` actualizado. No trabajar en `master`. **Skill:** `iniciar-rama` — invocar la cápsula paths.skillCapsules[\"iniciar-rama\"]: `scripts/skills/iniciar-rama/Iniciar-Rama.bat feat <nombre_feature>` o `Iniciar-Rama.ps1 -BranchType feat -BranchName "<nombre_feature>"`. Ver `SddIA/skills/iniciar-rama/spec.md`. |
