...
*   `--context`: **Parámetro de entrada obligatorio para la ruta de los ficheros.** Ruta base donde se generará el archivo de especificación. **Ha de venir dada por el agente documental** (Cúmulo). La acción Spec **solo decide el nombre del fichero** (ej. `SPEC-{SanitizedTitle}.md`); la ruta completa es `{context}/{NombreFichero}.md`.
    *   **Para fixes:** Usar el mismo patrón que en feature: **{persist} = AGENTE_CUMULO.fixPath/<nombre_fix>**. La ruta de salida es `{persist}/` (ej. paths.fixPath/<nombre_fix>/). El invocador obtiene {persist} del agente documental y la pasa como `--context`.
...
3.  **Determinación de Contexto:** La ruta de salida es **exclusivamente** el parámetro `--context` proporcionado por el invocador. En un fix, el invocador usa {persist} = paths.fixPath/<nombre_fix> (consultar Cúmulo paths.fixPath). No hay valor por defecto; la ruta ha de venir dada.
