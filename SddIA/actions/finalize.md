...
4. **Subir la rama (push):** Publicar la rama actual en el remoto. Ejecutar `git push -u origin <rama_actual>` (o equivalente vía skill finalizar-git / invoke-command). Este paso es obligatorio: finalize debe comprender que sin subir la rama no hay cierre completo; la rama debe quedar en `origin` antes de crear el PR.
